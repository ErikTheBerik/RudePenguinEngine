...
void VulkanFramework::CreateImageViews()
{
	...
	{
		mySwapChainImageViews[i] = CreateImageView(mySwapChainImages[i], mySwapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, myMipLevels);
	}
}
...
void VulkanFramework::CreateDepthResources()
{
	...
	CreateImage(mySwapChainExtent.width, mySwapChainExtent.height, myMipLevels, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, myDepthImage, myDepthImageMemory);
	myDepthImageView = CreateImageView(myDepthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, myMipLevels);

	TransitionImageLayout(myDepthImage, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, myMipLevels);
...
void VulkanFramework::CreateTextureImage()
{
	...
	CreateImage(texWidth, texHeight, myMipLevels, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		myTextureImage, myTextureImageMemory);
	
	TransitionImageLayout(myTextureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, myMipLevels);
	...
	GenerateMipmaps(myTextureImage, VK_FORMAT_R8G8B8A8_UNORM, texWidth, texHeight, myMipLevels);

	TransitionImageLayout(myTextureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, myMipLevels);

	vkDestroyBuffer(myLogicDevice, stagingBuffer, nullptr);
	vkFreeMemory(myLogicDevice, stagingBufferMemory, nullptr);
}

void VulkanFramework::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
...
{
	myTextureImageView = CreateImageView(myTextureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, myMipLevels);
}
...
void VulkanFramework::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
...
void VulkanFramework::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
	VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
VkImageView VulkanFramework::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
{
